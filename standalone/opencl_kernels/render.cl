        ...
        for (int i = y1; i < y2;i++){
            ...
            for (int j = ax; j <= bx; j++){
                ...
                if (tex_w > depth_buffer[pixelPosition]){
                    buffer_image[pixelPosition] = pixel;
                    depth_buffer[pixelPosition] = tex_w;
                }
...
// Print RGB Value 
void RGBPrint(RGB rgb){
    printf("(%f,%f,%f)\n",rgb.r,rgb.g,rgb.b);
}

// Print cl_TextureDetail
void cl_TextureDetailPrint(cl_TextureDetail detail){
    printf("(%i,%i,%i)\n",detail.width,detail.height,detail.start_index);
}
