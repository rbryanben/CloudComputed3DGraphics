...
struct Mesh
{
	...
	bool LoadFromObjectFile(string sFilename, Image texture=NULL)
	{
		...
		vector<cl_Vect3d> verts;
		vector<cl_Vect2d> texs;
		...
		while (!f.eof())
		{
			...
			else
			{
				if (line[0] == 'f')
				{
                    ...
                    cl_Triangle tri = { verts[stoi(tokens[0]) - 1], verts[stoi(tokens[2]) - 1], verts[stoi(tokens[4]) - 1],
...
void writeTrianglesToFile(std::vector<Triangle> &triangles, const std::string& filename) {
