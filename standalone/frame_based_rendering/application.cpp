...
class W3DGraphics {
    ...
    public:
        ...
        // Program
        cl::Program renderProgram;

        
        cl_Pixel_Texture_Out* prevArray;
        // OpenCL setup 
        void setupOpenCL(){
            this->renderProgram = createClProgram("opencl_kernels/render.cl");
            // Create the queue 
            this->context = this->renderProgram.getInfo<CL_PROGRAM_CONTEXT>();
            this->device =  this->context.getInfo<CL_CONTEXT_DEVICES>().front();

            cl::CommandQueue queue(this->context,this->device);
            this->clqueue = queue;

            // Kernel so that we can get the work group size
            cl::Kernel kernel(renderProgram,"render");
            this->renderKernel = kernel;

            prevArray = new cl_Pixel_Texture_Out[800*800];
        }
        ...
        void onWindowUpdate(){
            ...
            // Combine meshes 
            int textureCount = 0; // For assigning texture number to triangles 
            ...
            // Light 
            // Also move the light to use opencl
            light.render(combinedMeshesTriangles,true);

            /**
             * @brief OpenCl implementation starts here 
             */
            ...
            // In Buffers 
            cl::Buffer buffer_triangles_in(this->context,CL_MEM_READ_WRITE,sizeof(cl_Triangle)* combinedMeshesTriangleCount);
            cl::Buffer buffer_camera_matrix(this->context,CL_MEM_READ_WRITE,sizeof(cl_Matrix4x4));
            cl::Buffer buffer_projection_matrix(this->context,CL_MEM_READ_ONLY,sizeof(cl_Matrix4x4));
            cl::Buffer buffer_window_width(this->context,CL_MEM_READ_ONLY,sizeof(int));
            cl::Buffer buffer_window_height(this->context,CL_MEM_READ_ONLY,sizeof(int));
            cl::Buffer buffer_depth(this->context,CL_MEM_READ_WRITE,sizeof(float)*this->window_width * this->window_height);

            // Out Buffers 
            cl::Buffer buffer_triangles_out(this->context,CL_MEM_READ_WRITE,sizeof(cl_Triangle) * 32 * combinedMeshesTriangleCount);
            cl::Buffer buffer_pixel_texture_out(this->context,CL_MEM_READ_WRITE,sizeof(cl_Pixel_Texture_Out) * this->window_width * this->window_height);


            // Write buffers 
            this->clqueue.enqueueWriteBuffer(buffer_triangles_in,CL_TRUE,0,sizeof(cl_Triangle) * combinedMeshesTriangleCount,&h_combinedMeshesTriangleArray);
            this->clqueue.enqueueWriteBuffer(buffer_camera_matrix,CL_TRUE,0,sizeof(cl_Matrix4x4),&h_CameraMatrix);
            this->clqueue.enqueueWriteBuffer(buffer_projection_matrix,CL_TRUE,0,sizeof(cl_Matrix4x4),&h_projectionMatrix);
            this->clqueue.enqueueWriteBuffer(buffer_window_width,CL_TRUE,0,sizeof(int),&this->window_width);
            this->clqueue.enqueueWriteBuffer(buffer_window_height,CL_TRUE,0,sizeof(int),&this->window_height);
        

            // Set Kernel Argument 
            this->renderKernel.setArg(0,buffer_triangles_in);
            this->renderKernel.setArg(1,buffer_camera_matrix);
            this->renderKernel.setArg(2,buffer_projection_matrix);
            this->renderKernel.setArg(3,buffer_window_width);
            this->renderKernel.setArg(4,buffer_window_height);
            this->renderKernel.setArg(5,buffer_triangles_out);
            this->renderKernel.setArg(6,buffer_pixel_texture_out);
            this->renderKernel.setArg(7,buffer_depth);
            ...
            // Read all pixel outs
            cl_Pixel_Texture_Out* h_pixelTexturesOut = new cl_Pixel_Texture_Out[this->window_width * this->window_height]; 
            this->clqueue.enqueueReadBuffer(buffer_pixel_texture_out,CL_TRUE,0,sizeof(cl_Pixel_Texture_Out) * this->window_width * this->window_height,h_pixelTexturesOut);
            this->clqueue.finish();

            // Check if the prev and the current pixel array is the same 
            //for (int i=0; i != this->window_height * this->window_width;i++){
            //    if (this->prevArray[i].depth != h_pixelTexturesOut[i].depth){
            //        std::cout << "Buffers are not the same" << std::endl;
            //        break;
            //    }
            //}
            ...
            //Set prev as current
            //for (int i=0; i != this->window_height * this->window_width;i++){
            //    this->prevArray[i] = h_pixelTexturesOut[i];
            //}
            ...
            glBegin(GL_POINTS);
                ...
                for (int pos = 0 ; pos != this->window_width * this->window_height; pos++){
                    ...
                    if (h_pixelTexturesOut[pos].depth != 0){
                        int y = pos / this->window_width;
                        int x = pos - (y * this->window_width);
                        
                        // Pixel to draw 
                        cl_Pixel_Texture_Out pix =  h_pixelTexturesOut[pos];

                        ///
                        /// Working here
                        int textureHeight = this->textures[h_pixelTexturesOut[pos].textureNumber]->getHeight() - 1;
                        int textureWidth = this->textures[h_pixelTexturesOut[pos].textureNumber]->getWidth() - 1;
            ...
            // Using OpenCL determine if a triangle is visible or not 
            ...
            for (Triangle tri : combinedMeshesTriangles){
                ...
                if (VectorDotProduct(vCameraRay,normal) < 0
                    ...
                    //
                    // (Can be optimized)
                    // Holds projected triangles and their references 
